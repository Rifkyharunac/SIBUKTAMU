...
export function AdminLoginForm({ returnTo }: { returnTo: string }) {
  ...
  return <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#e0f2fe] px-4 py-10">
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_15%,rgba(14,165,233,0.15),transparent_28%),radial-gradient(circle_at_15%_90%,rgba(7,89,133,0.12),transparent_30%)]" />
    <section className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] border border-sky-950/10 bg-white shadow-[0_30px_90px_rgba(7,89,133,0.18)] lg:grid-cols-[0.9fr_1.1fr]">
      <div className="relative hidden overflow-hidden bg-[#0369a1] p-10 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="absolute -right-24 -top-24 size-72 rounded-full border-[48px] border-white/5" />
        <BrandMark inverse />
        <div className="relative mt-24"><span className="grid size-14 place-items-center rounded-2xl border border-white/15 bg-white/10"><ShieldCheck className="size-7 text-sky-200" /></span><h1 className="mt-6 text-3xl font-black leading-tight">Portal Petugas<br />SIBUKTAMU</h1><p className="mt-4 max-w-sm text-sm leading-7 text-sky-100/75">Akses internal untuk memantau kedatangan, memperbarui pelayanan, dan menyusun laporan resmi.</p></div>
      ...
      <div className="p-6 sm:p-10 lg:p-12">
        ...
        <form onSubmit={submit} className="mt-7 space-y-5">
          ...
          <Button type="submit" disabled={loading} className="h-12 w-full bg-[#0369a1] text-base font-extrabold hover:bg-[#075985]">{loading ? <><LoaderCircle className="animate-spin" />Memeriksa Akun…</> : "Masuk ke Dashboard"}</Button>
        </form>
