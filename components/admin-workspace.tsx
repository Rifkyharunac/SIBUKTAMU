...
export function AdminWorkspace({ section, initialIdentity, focusedVisitId }: { section: string; initialIdentity: AdminIdentity; focusedVisitId?: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#f0f9ff] text-slate-950">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r border-sky-950/30 bg-[#0369a1] text-white shadow-2xl shadow-sky-950/10 transition-transform lg:translate-x-0 ${mobileMenu ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="lg:pl-[280px]">
        <header className="sticky top-0 z-20 flex min-h-20 items-center justify-between gap-4 border-b border-slate-200/90 bg-white/95 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3"><button className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white shadow-sm lg:hidden" onClick={() => setMobileMenu(true)} aria-label="Buka menu"><Menu className="size-5" /></button><div className="min-w-0"><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#0369a1]">Administrasi Internal</p><h1 className="truncate text-xl font-black tracking-tight text-slate-950 sm:text-2xl">{sectionTitles[section] ?? "Dashboard Pelayanan"}</h1></div></div>
