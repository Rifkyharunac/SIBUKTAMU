...
export function AdminWorkspace({ section, initialIdentity, focusedVisitId }: { section: string; initialIdentity: AdminIdentity; focusedVisitId?: string }) {
  ...
  return (
    <div className="admin-workspace min-h-screen bg-[#f0f9ff] text-slate-950">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[min(280px,88vw)] flex-col border-r border-sky-950/30 bg-[#0369a1] text-white shadow-2xl shadow-sky-950/10 transition-transform lg:translate-x-0 ${mobileMenu ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="scrollbar-thin flex-1 space-y-1 overflow-y-auto px-3 py-5">
          <p className="mb-3 px-3 text-[10px] font-extrabold uppercase tracking-[0.2em] text-sky-200/70">Menu Administrasi</p>
          {menu.map((item) => {
            const Icon = item.icon;
            const href = item.href ?? `/admin/${item.key}`;
            const active = section === item.key;
            return <Link key={item.key} href={href} aria-current={active ? "page" : undefined} onClick={() => setMobileMenu(false)} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-white text-[#0369a1] shadow-lg shadow-black/10" : "text-sky-50/80 hover:bg-white/10 hover:text-white"}`}><span className={`grid size-8 place-items-center rounded-lg ${active ? "bg-sky-100 text-[#0369a1]" : "bg-white/8 text-sky-100 group-hover:bg-white/12"}`}><Icon className="size-[18px]" /></span>{item.label}{item.key === "notifikasi" && pendingNotifications > 0 && <span className="ml-auto grid min-w-5 place-items-center rounded-full bg-amber-400 px-1 text-[10px] font-black text-sky-950">{pendingNotifications}</span>}</Link>;
          })}
        </nav>
        <div className="border-t border-white/10 bg-black/10 p-4">
          <div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-sky-200 text-sm font-black text-sky-950">{initialIdentity.name.slice(0, 1).toUpperCase()}</span><div className="min-w-0"><p className="truncate text-sm font-extrabold text-white">{initialIdentity.name}</p><p className="truncate text-xs text-sky-200">{initialIdentity.roleLabel}</p></div></div>
          <LogoutButton className="text-sky-100 hover:bg-white/10 hover:text-white" />
        </div>
      </aside>
      ...
      <div className="lg:pl-[280px]">
        <header className="sticky top-0 z-20 flex min-h-20 items-center justify-between gap-2 border-b border-slate-200/90 bg-white/95 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-3"><button className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white shadow-sm lg:hidden" onClick={() => setMobileMenu(true)} aria-label="Buka menu"><Menu className="size-5" /></button><div className="min-w-0"><p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#0369a1]">Administrasi Internal</p><h1 className="text-base leading-tight font-black tracking-tight text-slate-950 sm:text-2xl">{sectionTitles[section] ?? "Dashboard Pelayanan"}</h1></div></div>
          <div className="flex shrink-0 items-center gap-2"><div className="mr-2 hidden text-right xl:block"><p className="text-xs font-extrabold text-slate-800">{initialIdentity.name}</p><p className="text-[11px] text-slate-500">{initialIdentity.roleLabel}</p></div><Button variant="outline" size="icon" className="rounded-xl bg-white shadow-sm" title="Muat ulang" onClick={() => load()}><RefreshCw className={loading ? "animate-spin" : ""} /></Button>{menu.some(item => item.key === "notifikasi") && <Button asChild variant="outline" size="icon" className="relative rounded-xl bg-white shadow-sm"><Link href="/admin/notifikasi" onClick={() => setMobileMenu(false)} aria-label={`${pendingNotifications} notifikasi belum dibaca`}><Bell />{pendingNotifications > 0 && <span className="absolute -right-1 -top-1 size-2.5 rounded-full bg-rose-600 ring-2 ring-white" />}</Link></Button>}</div>
        </header>
...
function DashboardSection({ data, selectVisit }: { data: Overview; selectVisit: (visit: Visit) => void }) {
  ...
  return <div className="space-y-6">
    <div className="relative overflow-hidden rounded-2xl bg-[#0369a1] p-5 text-white shadow-lg shadow-sky-950/10 sm:p-6">
      <div className="absolute -right-16 -top-20 size-56 rounded-full border-[36px] border-white/5" />
      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-sky-200">Ringkasan Operasional</p><h2 className="mt-2 text-xl font-black tracking-tight sm:text-2xl">Seluruh Layanan Disnakertrans</h2><p className="mt-1.5 max-w-2xl text-sm leading-6 text-sky-50/75">Monitoring kunjungan pada lima Kabid, dua UPT Wasnaker, dan Sekretariat Dinas.</p></div>
  ...
  return <div className="space-y-5">
    <Panel title="Filter Data Kunjungan" subtitle="Temukan data berdasarkan identitas tamu, status pelayanan, atau bidang tujuan.">
      <div className="grid items-end gap-3 md:grid-cols-[minmax(240px,1fr)_190px_240px_auto]">
        ...
        <Button className="h-11 bg-[#0369a1] px-5 hover:bg-[#075985]" onClick={() => props.load()}><Search />Tampilkan</Button>
      </div>
    </Panel>
...
function VisitSheet({ visit, departments, onClose, action }: { visit: Visit | null; departments: Department[]; onClose: () => void; action: (payload: Record<string, unknown>, message?: string) => Promise<boolean> }) {
  ...
  return <Sheet open={!!visit} onOpenChange={(open) => !open && onClose()}><SheetContent className="w-full overflow-y-auto sm:max-w-xl"><SheetHeader className="border-b"><SheetTitle className="text-xl">Detail Kunjungan</SheetTitle><SheetDescription>{visit?.visitCode}</SheetDescription></SheetHeader>{visit && <div className="space-y-6 p-5">
    ...
    <div><p className="mb-3 text-sm font-bold">Tindakan pelayanan</p><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => update("DITERIMA")}>Terima tamu</Button><Button variant="outline" onClick={() => update("SEDANG_DILAYANI")}>Layani</Button><Button className="bg-[#0369a1] hover:bg-[#075985]" onClick={() => update("SELESAI")}>Selesaikan</Button><Button variant="destructive" onClick={() => update("BATAL")}>Batalkan</Button></div></div>
...
function DepartmentSection({ data, action }: { data: Overview; action: (payload: Record<string, unknown>, message?: string) => Promise<boolean> }) {
  ...
  return <div className="grid items-start gap-6 2xl:grid-cols-[440px_minmax(0,1fr)]">
    <Editor title={form.id ? "Ubah Data Bidang" : "Tambah Bidang"} onSave={save} onReset={() => setForm(empty)}>
      <FormField label="Kode Bidang"><Input placeholder="Contoh: P5TK" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} /></FormField>
      ...
      <FormField label="Kontak bidang" hint="Kontak referensi. Penerima notifikasi diatur melalui akun pada menu Pengguna."><Input inputMode="tel" placeholder="08xxxxxxxxxx" value={form.whatsappNumber} onChange={(e) => setForm({ ...form, whatsappNumber: e.target.value })} /></FormField>
...
function QrSection() {
  ...
  useEffect(() => {
    ...
    QRCode.toString(target, { type: "svg", margin: 2, errorCorrectionLevel: "H", color: { dark: "#0369a1", light: "#ffffff" } }).then(setSvg);
  }, [target]);
...
function NotificationSection({ data, action }: { data: Overview; action: (payload: Record<string, unknown>, message?: string) => Promise<boolean> }) {
  ...
  return <div className="space-y-5">
    <Panel title="Pusat Notifikasi Kunjungan" subtitle="Pemberitahuan tamu datang dan layanan selesai. Status dibaca tersimpan untuk akun masing-masing.">
      ...
      <div className="space-y-3">{items.length ? items.map((item) => <article key={item.id} className={`rounded-2xl border bg-white p-4 transition sm:p-5 ${item.isRead ? "border-slate-200" : "border-sky-300 shadow-[0_8px_24px_rgba(2,132,199,0.08)]"}`}>
...
function CheckLabel({ label, checked, setChecked }: { label: string; checked: boolean; setChecked: (value: boolean) => void }) { return <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3.5 text-sm font-bold text-slate-700 transition hover:border-sky-200 hover:bg-sky-50/50"><Checkbox checked={checked} onCheckedChange={(value) => setChecked(value === true)} />{label}</label>; }
function Panel({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) { return <section className="admin-panel rounded-2xl border border-[#dbeafe] bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_28px_rgba(15,23,42,0.035)] sm:p-6"><div className="mb-5 border-b border-slate-100 pb-4"><h2 className="text-base font-black tracking-tight text-slate-950 sm:text-lg">{title}</h2>{subtitle && <p className="mt-1.5 text-sm leading-6 text-slate-500">{subtitle}</p>}</div>{children}</section>; }
function StatCard({ label, value, icon: Icon, tone }: { label: string; value: string | number; icon: React.ComponentType<{ className?: string }>; tone: string }) { const colors: Record<string, string> = { emerald: "bg-sky-50 text-sky-700 ring-sky-100", amber: "bg-amber-50 text-amber-700 ring-amber-100", blue: "bg-blue-50 text-blue-700 ring-blue-100", violet: "bg-violet-50 text-violet-700 ring-violet-100", slate: "bg-slate-100 text-slate-700 ring-slate-200" }; return <article className="stat-card relative overflow-hidden rounded-2xl border border-[#dbeafe] bg-white p-4 sm:p-5 shadow-[0_6px_22px_rgba(15,23,42,0.04)]"><span className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#0369a1] to-sky-300" /><div className="flex items-start justify-between gap-3"><div><p className="text-3xl font-black tracking-tight text-slate-950">{value}</p><p className="mt-2 text-[11px] font-extrabold uppercase leading-4 tracking-[0.11em] text-slate-500">{label}</p></div><div className={`grid size-11 shrink-0 place-items-center rounded-xl ring-1 ${colors[tone]}`}><Icon className="size-5" /></div></div></article>; }
function StatusBadge({ status }: { status: string }) { const map: Record<string, string> = { BARU: "bg-blue-100 text-blue-800", MENUNGGU: "bg-amber-100 text-amber-800", DITERIMA: "bg-cyan-100 text-cyan-800", SEDANG_DILAYANI: "bg-violet-100 text-violet-800", SELESAI: "bg-sky-100 text-sky-800", DIALIHKAN: "bg-orange-100 text-orange-800", BATAL: "bg-rose-100 text-rose-800", ACCEPTED: "bg-cyan-100 text-cyan-800", SENT: "bg-sky-100 text-sky-800", FAILED: "bg-rose-100 text-rose-800", QUEUED: "bg-amber-100 text-amber-800", NOT_CONFIGURED: "bg-slate-100 text-slate-700" }; return <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide ${map[status] ?? "bg-slate-100 text-slate-700"}`}>{statusLabel(status)}</span>; }
function statusLabel(status: string) { const labels: Record<string, string> = { SENDING: "Sedang dikirim", ACCEPTED: "Diterima Penyedia", SENT: "Terkirim", FAILED: "Gagal Terkirim", QUEUED: "Dalam Antrean", NOT_CONFIGURED: "WhatsApp Belum Aktif" }; return labels[status] ?? status.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase()); }
...
function LoadingState() { return <div className="grid min-h-[50vh] place-items-center text-sm text-slate-500"><div className="text-center"><LoaderCircle className="mx-auto mb-3 size-7 animate-spin text-[#0369a1]" />Memuat data pelayanan…</div></div>; }
