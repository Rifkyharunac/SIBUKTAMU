      ...
      <div className="space-y-3">{items.length ? items.map((item) => <article key={item.id} className={`rounded-2xl border bg-white p-4 transition sm:p-5 ${item.isRead ? "border-slate-200" : "border-emerald-300 shadow-[0_8px_24px_rgba(5,150,105,0.08)]"}`}>
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2 xl:justify-end">
            ...
            {!["SENT", "ACCEPTED"].includes(item.status) && canRetryAutomatically && <Button variant="outline" size="sm" className="border-emerald-200 text-emerald-800 hover:bg-emerald-50" onClick={() => action({ action: "RETRY_WHATSAPP", id: item.id }, "Pengiriman ulang telah diproses.")}><RefreshCw />Kirim Ulang</Button>}
            {!["SENT", "ACCEPTED"].includes(item.status) && !canRetryAutomatically && item.recipient && <Button asChild variant="outline" size="sm" className="border-emerald-200 text-emerald-800 hover:bg-emerald-50"><a href={manualLink(item)} target="_blank" rel="noreferrer"><MessageCircle />Kirim Manual</a></Button>}
...
function statusLabel(status: string) { const labels: Record<string, string> = { ACCEPTED: "Diterima Penyedia", SENT: "Terkirim", FAILED: "Gagal Terkirim", QUEUED: "Dalam Antrean", NOT_CONFIGURED: "WhatsApp Belum Aktif" }; return labels[status] ?? status.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase()); }
