  ...
  return <div className="space-y-6">
    <div className="relative overflow-hidden rounded-2xl bg-[#063d2f] p-5 text-white shadow-lg shadow-emerald-950/10 sm:p-6">
      ...
      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-emerald-200">Ringkasan Operasional</p><h2 className="mt-2 text-xl font-black tracking-tight sm:text-2xl">Seluruh Layanan Disnakertrans</h2><p className="mt-1.5 max-w-2xl text-sm leading-6 text-emerald-50/75">Monitoring kunjungan pada lima Kabid, dua UPT Wasnaker, dan Sekretariat Dinas.</p></div>
...
function ServiceSection({ data, action }: { data: Overview; action: (payload: Record<string, unknown>, message?: string) => Promise<boolean> }) {
  ...
  return <div className="grid items-start gap-6 2xl:grid-cols-[460px_minmax(0,1fr)]">
    <Editor title={form.id ? "Ubah Data Layanan" : "Tambah Layanan"} onSave={save} onReset={() => setForm(empty)}>
      ...
      <FormField label="WhatsApp Layanan" hint="Kontak referensi. Notifikasi dikirim ke akun admin bidang dan super admin aktif."><Input inputMode="tel" placeholder="08xxxxxxxxxx" value={form.whatsappNumber} onChange={(e) => setForm({ ...form, whatsappNumber: e.target.value })} /></FormField>
...
function UserSection({ data, action }: { data: Overview; action: (payload: Record<string, unknown>, message?: string) => Promise<boolean> }) {
  ...
  return <div className="grid items-start gap-6 2xl:grid-cols-[460px_minmax(0,1fr)]">
    <Editor title={form.id ? "Ubah Data Pengguna" : "Tambah Pengguna"} onSave={save} onReset={() => setForm(empty)}>
      ...
      <FormField label="Nomor WhatsApp" hint="Semua admin aktif menerima notifikasi setiap kunjungan. Gunakan nomor penerima yang berbeda dari nomor bot."><Input inputMode="tel" placeholder="08xxxxxxxxxx" value={form.whatsappNumber} onChange={(e) => setForm({ ...form, whatsappNumber: e.target.value })} /></FormField>
...
function ReportSection({ data, selectVisit }: { data: Overview; selectVisit: (visit: Visit) => void }) {
  ...
  return <div className="space-y-6">
    <Panel title="Laporan kunjungan dinas" subtitle="Kop berlogo, periode, tabel lengkap, total kunjungan, dan ruang pengesahan. Maksimal 2.000 kunjungan per ekspor.">
      <div className="mb-5 grid gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950 sm:grid-cols-3"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Instansi</p><p className="mt-1 font-bold">Disnakertrans Provinsi Sulawesi Tengah</p></div><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Lingkup</p><p className="mt-1 font-bold">Bidang, UPT, Sekretariat & Penerima Tamu</p></div><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Orientasi PDF</p><p className="mt-1 font-bold">A4 Landscape</p></div></div>
      ...
      <div className="space-y-3">{items.length ? items.map((item) => <article key={item.id} className={`rounded-2xl border bg-white p-4 transition sm:p-5 ${item.isRead ? "border-slate-200" : "border-emerald-300 shadow-[0_8px_24px_rgba(5,150,105,0.08)]"}`}>
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2 xl:justify-end">
            ...
            {!["SENT", "ACCEPTED", "SENDING"].includes(item.status) && canRetryAutomatically && <Button variant="outline" size="sm" className="border-emerald-200 text-emerald-800 hover:bg-emerald-50" onClick={() => action({ action: "RETRY_WHATSAPP", id: item.id }, "Pengiriman ulang telah diproses.")}><RefreshCw />Kirim Ulang</Button>}
            {!["SENT", "ACCEPTED", "SENDING"].includes(item.status) && !canRetryAutomatically && item.recipient && <Button asChild variant="outline" size="sm" className="border-emerald-200 text-emerald-800 hover:bg-emerald-50"><a href={manualLink(item)} target="_blank" rel="noreferrer"><MessageCircle />Kirim Manual</a></Button>}
