...
export function AdminApp({userId,onRefresh}:{userId:string;onRefresh:()=>void}){
 ...
 useEffect(()=>{
  ...
  if('serviceWorker' in navigator)navigator.serviceWorker.register('/admin/sw.js',{scope:'/admin/',updateViaCache:'none'}).then(async reg=>{
   await reg.update();
 ...
 async function testPush(){setBusy(true);setMessage('');try{const reg=await navigator.serviceWorker.getRegistration('/admin/');const sub=await reg?.pushManager.getSubscription();if(!sub)throw new Error('Aktifkan notifikasi perangkat terlebih dahulu.');await apiRequest('/api/admin/device',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'TEST',subscription:sub.toJSON()})});setMessage('Layanan push menerima pesan uji. Periksa bilah notifikasi HP. Jika tidak muncul atau tidak berbunyi, izinkan notifikasi dan suara SIBUKTAMU pada pengaturan HP.');}catch(e){setMessage(e instanceof Error?e.message:'Uji notifikasi gagal.');}finally{setBusy(false);}}
 ...
 return <section className="mb-5 rounded-2xl border border-sky-200 bg-white p-4" aria-label="Aplikasi dan pemberitahuan admin">
  <div className="flex items-center gap-3"><img src="/sibuktamu-icon-192.png?v=3" alt="" className="size-11 rounded-xl"/><div className="min-w-0 flex-1"><p className="font-bold text-sky-950">Aplikasi SIBUKTAMU</p><p className="text-sm text-slate-600">{push?"Notifikasi perangkat aktif":"Notifikasi HP belum diaktifkan"}</p></div><Button variant="outline" aria-expanded={expanded} onClick={()=>setExpanded(!expanded)}>{expanded?"Tutup":"Atur"}</Button></div><div className={`${expanded?"grid":"hidden"} mt-4 grid-cols-1 gap-2 sm:flex sm:flex-wrap [&_button]:min-h-11 [&_button]:whitespace-normal`}>
   ...
   {push&&<Button variant="outline" disabled={busy} onClick={testPush}>Uji notifikasi HP</Button>}
