...
export function AdminApp({userId,onRefresh}:{userId:string;onRefresh:()=>void}){
 ...
 useEffect(()=>{
  const link=document.createElement('link');link.rel='manifest';link.href='/admin/app.webmanifest';document.head.appendChild(link);
 ...
 return <section className="mb-5 rounded-2xl border border-sky-200 bg-white p-4" aria-label="Aplikasi dan pemberitahuan admin">
  <div className="flex flex-wrap items-center gap-2"><p className="mr-auto font-bold text-sky-950">Pemberitahuan tamu</p>
   {!installed&&<Button variant="outline" onClick={async()=>{if(install){await install.prompt();setInstall(null);}else setMessage(ios?'Di Safari, tekan Bagikan → Tambahkan ke Layar Utama, lalu buka aplikasinya untuk mengaktifkan notifikasi.':'Buka menu browser → Instal aplikasi / Tambahkan ke layar utama. Jika pilihan belum muncul, gunakan Chrome atau Edge terbaru.');}}><Download/>Instal aplikasi admin</Button>}
   <Button variant="outline" onClick={toggleSound}>{sound?<Volume2/>:<VolumeX/>}{sound?'Matikan suara':'Aktifkan suara'}</Button>
   <Button disabled={busy} onClick={push?disablePush:enablePush}><Bell/>{busy?'Memproses…':push?'Nonaktifkan notifikasi HP':'Aktifkan notifikasi HP'}</Button>
