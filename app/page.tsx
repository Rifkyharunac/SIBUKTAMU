...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#f0f9ff] text-slate-950">
      ...
      <section className="hero-3d-shell relative border-b border-sky-900/10">
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 sm:py-20 lg:min-h-[720px] lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:py-24">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/12 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-white shadow-lg backdrop-blur-md"><Clock3 className="size-4 text-sky-200" />Mudah diisi dari ponsel</div>
            <div className="mt-6 flex items-center gap-3">
              ...
              <div><p className="text-sm font-extrabold uppercase tracking-[0.22em] text-sky-100">SIBUKTAMU</p><p className="mt-1 text-xs font-semibold tracking-wide text-sky-200">Buku Tamu Digital · Disnakertrans</p></div>
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-sky-50/90">
              ...
              <span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-sky-200" />Tanpa NIK/KTP</span>
              <span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-sky-200" />Tanda tangan digital</span>
            </div>
          </div>
          ...
          <div className="relative mx-auto w-full max-w-lg [perspective:1200px]">
            ...
            <div className="relative rounded-[2rem] border border-white/40 bg-white p-6 shadow-[16px_24px_0_rgba(7,89,133,.2),0_45px_90px_rgba(0,0,0,.35)] sm:p-8">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-5"><img src="/logo-sulteng-small.png" width="60" height="60" alt="Lambang Sulawesi Tengah" /><div><p className="text-xs font-bold uppercase tracking-widest text-sky-700">Selamat datang</p><h2 className="mt-1 text-2xl font-black text-slate-900">Buku tamu digital</h2></div></div>
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#0369a1]">Layanan resmi</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Layanan ketenagakerjaan dan transmigrasi dalam satu pintu.</h2><p className="mt-4 leading-7 text-slate-600">Pilih bidang dan keperluan saat mengisi buku tamu. Sistem meneruskan data kepada bidang penanggung jawab sesuai struktur organisasi.</p></div>
            <div className="grid gap-3 sm:grid-cols-2">{officialDepartments.map((department, index) => <div key={department} className="group flex items-center gap-3 rounded-2xl border border-slate-200 bg-[#f8fcff] p-4 transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-lg"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-sky-100 text-sm font-black text-[#0369a1]">{String(index + 1).padStart(2, "0")}</span><p className="font-bold text-slate-800">{department}</p></div>)}</div>
          </div>
          <div className="mt-14 grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 p-5"><Building2 className="size-6 text-[#0369a1]" /><h3 className="mt-4 font-black">Bidang tujuan jelas</h3><p className="mt-2 text-sm leading-6 text-slate-600">Layanan langsung diarahkan ke bidang penanggung jawab yang tepat.</p></div>
            <div className="rounded-2xl border border-slate-200 p-5"><Bell className="size-6 text-[#0369a1]" /><h3 className="mt-4 font-black">Notifikasi petugas</h3><p className="mt-2 text-sm leading-6 text-slate-600">Petugas menerima detail kedatangan agar pelayanan lebih responsif.</p></div>
            <div className="rounded-2xl border border-slate-200 p-5"><ShieldCheck className="size-6 text-[#0369a1]" /><h3 className="mt-4 font-black">Data terkelola</h3><p className="mt-2 text-sm leading-6 text-slate-600">Kunjungan tercatat, dapat dipantau, dan siap direkap untuk laporan dinas.</p></div>
          </div>
        </div>
      </section>

      <footer className="border-t border-slate-200 bg-[#f0f9ff]">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-7 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <a href="mailto:disnakertrans@sultengprov.go.id" className="font-semibold text-[#0369a1]">disnakertrans@sultengprov.go.id</a><p>Pelayanan informasi: Senin–Jumat, 08.00–15.00 WITA</p><Link href="/privacy" className="font-semibold text-[#0369a1]">Kebijakan Privasi</Link>
        </div>
      </footer>
    </main>
  );
