...
export async function POST(request: Request) {
  try {
    const payload = await request.json() as {visitCode?:string;token?:string;phone?:string;action?:string;rating?:number;feedback?:string};
    ...
    const authorized = visit && ((token && visit.checkoutTokenHash === await stableHash(token)) || (phone.length >= 10 && phone === visit.phone));
    ...
    if (payload.action === "STATUS") return Response.json({visitCode:code,status:visit.status,checkOutAt:visit.checkOutAt,durationMinutes:visit.durationMinutes});
